...
public class PackageLib extends OneArgFunction
{
	...
	{
		LuaString argName = arg.checkstring();
		LuaValue loaded = LOADED.get(argName);
		if(loaded.toboolean())
		{
			if(loaded == _SENTINEL)
			    error("loop or previous error loading module '" + argName + "'");
			return loaded;
		}
		...
		for(int i = 1;; i++)
		{
			...
			if(loader.isnil())
			    error("module '" + argName + "' not found: " + argName + sb);

			/* call loader with module name as argument */
			chunk = loader.call(argName);
			if(chunk.isfunction())
			    break;
			if(chunk.isstring())
		...
		LOADED.set(argName, _SENTINEL);
		@SuppressWarnings("null")
		LuaValue result = chunk.call(argName);
		if(!result.isnil())
			LOADED.set(argName, result);
		else if((result = LOADED.get(argName)) == _SENTINEL)
		    LOADED.set(argName, result = LuaValue.TRUE);
		return result;
	}
	...
	{
		LuaString argName = args.checkstring(1);
		LuaValue preload = PACKAGE.get(_PRELOAD).checktable();
		LuaValue val = preload.get(argName);
		return val.isnil() ? valueOf("\n\tno field package.preload['" + argName + "']") : val;
	}

	LuaValue loader_Lua(Varargs args)
	{
		String argName = args.checkjstring(1);
		...
		while(e < n)
		{
			...
			if(q >= 0)
			{
				filename = template.substring(0, q) + argName + template.substring(q + 1);
			}
		...
		return valueOf(sb != null ? sb.toString() : "");
	}

	LuaValue loader_Java(Varargs args)
	{
		String classname = toClassname(args.checkjstring(1));
		Class<?> c = null;
		LuaValue v = null;
		try
		{
